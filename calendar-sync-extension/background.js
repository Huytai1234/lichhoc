...
function showNotification(title, message, type = 'basic', idSuffix = Date.now().toString()) { const notificationId = `uel-sync-notif-${idSuffix}`; let iconUrl = '1.png'; logger.debug(`BG Notif [${idSuffix}]: Showing: Title='${title}', Msg='${message.substring(0, 100)}...'`); chrome.notifications.create(notificationId, { type: 'basic', iconUrl: iconUrl, title: title, message: message, priority: 1 }, (createdId) => { if (chrome.runtime.lastError) logger.error(`BG Notif [${idSuffix}]: Create Error:`, chrome.runtime.lastError?.message); }); }
...
async function handleSemesterSync(userId, sendResponse) {
    ...
    try {
        ...
        for (let i = 0; i < weekOptions.length; i++) {
            ...
            showNotification(notificationTitle, `Bắt đầu xử lý tuần ${weekNumStr}: ${week.text}`, 'basic', `sem-week-start-${i}-${uniqueId}`);
